...
class Category extends CActiveRecord
{
    ...
    public function relations()
    {
        ...
        return array(
            ...
            'children' => [self::HAS_MANY, 'Category', 'parent_id', 'order' => 'children.name'],
    ...
    public function getCached($code)
    {
        ...
        if (!$category) {
            $category = $this->with('children', 'parent')->find('t.code=:code OR t.url=:code OR REPLACE(t.url, " ", "-")=:code', ['code'=>$code]);
    ...
    public function getCategorys($parent_id = 0)
    {
        ...
        $list = $this->cache(Yii::app()->params->cacheTime, $dependency)->findAllByAttributes(['parent_id' => $parent_id], ['order' => 't.name']);
    ...
    {
        $dependency = new CDbCacheDependency('SELECT MAX(update_time) FROM Category');
        $categorys = $this->cache(Yii::app()->params->cacheTime, $dependency)->with('children')->findAllByAttributes(['parent_id' => 0], ['order' => 't.name']);
        ...
        foreach ($categorys as $category) {
            ...
            foreach($category->children as $child) {
                $children[] = [
                    'text' => CHtml::link($child->name, ['category/index', 'code'=>$child->getUrl()]),
                ];
            }

            $data[] = [
                'text' => CHtml::link($category->name, ['category/index', 'code'=>$category->getUrl()]),
                'children' => $children
            ];
    ...
    public function getList($searchName = null)
    {
        ...
        $dataProvider = new CActiveDataProvider($this->cache(Yii::app()->params->cacheTime, null, 2), [
            'criteria' => $criteria,
            'pagination' => false,
        ]);
    ...
    public function getSeoDescription()
    {
        ...
        if (isset($this->categorys[0])) {
            $subcategoryName = $this->categorys[0]->url;
        }
